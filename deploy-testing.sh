...
# REMOTE_DIST_PATH="/<APPNAME>/testing/public/content/fluxguide/exhibitions/1/system/app/dist/"

# OPTIONAL: enable clean git check
...
# BUILDING THE APP (in testing mode)
npm run buildTesting

build_end=`date +%s`
buildtime=$((build_end-start))
echo "-------------------------------------------------------"
echo "deploy.sh took $buildtime seconds to build the vue app"
echo "-------------------------------------------------------"
...
runtime=$((end-start))
echo "----------------------------------------------------"
echo "deploy.sh took $runtime seconds for the build and upload"
echo "----------------------------------------------------"